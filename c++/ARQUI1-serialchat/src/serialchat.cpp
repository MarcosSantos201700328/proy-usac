...
serialchat::serialchat()
    : QMainWindow( 0, "serialchat", WDestructiveClose )
{
...
void serialchat::newDoc()
{
    serialchat *ed = new serialchat;
    ed->setCaption(tr("Qt Example - Application"));
    ed->show();
}

void serialchat::choose()
{
...
void serialchat::load( const QString &fileName )
{
    QFile f( fileName );
...
void serialchat::closeEvent( QCloseEvent* ce )
{
    if ( !e->isModified() ) {
	ce->accept();
	return;
