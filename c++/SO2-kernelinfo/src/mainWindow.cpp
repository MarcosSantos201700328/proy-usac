/****************************************************************************
** Form implementation generated from reading ui file 'mainWindow.ui'
**
** Created: Wed Mar 26 22:06:24 2008
**      by: The User Interface Compiler ($Id: qt/main.cpp   3.3.7   edited Aug 31 2005 $)
**
** WARNING! All changes made in this file will be lost!
****************************************************************************/

#include "mainWindow.h"

#include <qvariant.h>
#include <qpushbutton.h>
#include <qtabwidget.h>
#include <qwidget.h>
#include <qtable.h>
#include <qgroupbox.h>
#include <qlabel.h>
#include <qlineedit.h>
#include <qcheckbox.h>
#include <qtextbrowser.h>
#include <qlayout.h>
#include <qtooltip.h>
#include <qwhatsthis.h>
#include <qaction.h>
#include <qmenubar.h>
#include <qpopupmenu.h>
#include <qtoolbar.h>
#include <qimage.h>
#include <qpixmap.h>

static const unsigned char image0_data[] = { 
    
};

static const unsigned char image1_data[] = { 
    
};

static const unsigned char image2_data[] = { 
    
};

static const unsigned char image3_data[] = { 
    
};

static const unsigned char image4_data[] = { 
    
};

static const unsigned char image5_data[] = { 
    
};

static const unsigned char image6_data[] = { 
    
};

static const unsigned char image7_data[] = { 
    
};

static const unsigned char image8_data[] = { 
    
};

static const unsigned char image9_data[] = { 
    
};


/*
 *  Constructs a frm_main as a child of 'parent', with the
 *  name 'name' and widget flags set to 'f'.
 *
 */
frm_main::frm_main( QWidget* parent, const char* name, WFlags fl )
    : QMainWindow( parent, name, fl )
{
    (void)statusBar();
    QImage img;
    img.loadFromData( image0_data, sizeof( image0_data ), "PNG" );
    image0 = img;
    img.loadFromData( image1_data, sizeof( image1_data ), "PNG" );
    image1 = img;
    img.loadFromData( image2_data, sizeof( image2_data ), "PNG" );
    image2 = img;
    img.loadFromData( image3_data, sizeof( image3_data ), "PNG" );
    image3 = img;
    img.loadFromData( image4_data, sizeof( image4_data ), "PNG" );
    image4 = img;
    img.loadFromData( image5_data, sizeof( image5_data ), "PNG" );
    image5 = img;
    img.loadFromData( image6_data, sizeof( image6_data ), "PNG" );
    image6 = img;
    img.loadFromData( image7_data, sizeof( image7_data ), "PNG" );
    image7 = img;
    img.loadFromData( image8_data, sizeof( image8_data ), "PNG" );
    image8 = img;
    img.loadFromData( image9_data, sizeof( image9_data ), "PNG" );
    image9 = img;
    if ( !name )
	setName( "frm_main" );
    setCentralWidget( new QWidget( this, "qt_central_widget" ) );
    frm_mainLayout = new QVBoxLayout( centralWidget(), 11, 6, "frm_mainLayout"); 

    tabs = new QTabWidget( centralWidget(), "tabs" );

    tab = new QWidget( tabs, "tab" );
    tabLayout = new QVBoxLayout( tab, 11, 6, "tabLayout"); 

    tableProcess = new QTable( tab, "tableProcess" );
    tableProcess->setNumCols( tableProcess->numCols() + 1 );
    tableProcess->horizontalHeader()->setLabel( tableProcess->numCols() - 1, tr( "PID" ) );
    tableProcess->setNumCols( tableProcess->numCols() + 1 );
    tableProcess->horizontalHeader()->setLabel( tableProcess->numCols() - 1, tr( "Nombre" ) );
    tableProcess->setNumCols( tableProcess->numCols() + 1 );
    tableProcess->horizontalHeader()->setLabel( tableProcess->numCols() - 1, tr( "Status" ) );
    tableProcess->setNumCols( tableProcess->numCols() + 1 );
    tableProcess->horizontalHeader()->setLabel( tableProcess->numCols() - 1, tr( "Memoria" ) );
    tableProcess->setResizePolicy( QTable::AutoOneFit );
    tableProcess->setNumRows( 0 );
    tableProcess->setNumCols( 4 );
    tableProcess->setRowMovingEnabled( FALSE );
    tableProcess->setReadOnly( TRUE );
    tableProcess->setSelectionMode( QTable::SingleRow );
    tabLayout->addWidget( tableProcess );

    pushButtonKill = new QPushButton( tab, "pushButtonKill" );
    tabLayout->addWidget( pushButtonKill );
    tabs->insertTab( tab, QString::fromLatin1("") );

    tab_2 = new QWidget( tabs, "tab_2" );
    tabLayout_2 = new QVBoxLayout( tab_2, 11, 6, "tabLayout_2"); 

    tableModule = new QTable( tab_2, "tableModule" );
    tableModule->setNumCols( tableModule->numCols() + 1 );
    tableModule->horizontalHeader()->setLabel( tableModule->numCols() - 1, tr( "Modulo" ) );
    tableModule->setNumCols( tableModule->numCols() + 1 );
    tableModule->horizontalHeader()->setLabel( tableModule->numCols() - 1, tr( "Tam." ) );
    tableModule->setNumCols( tableModule->numCols() + 1 );
    tableModule->horizontalHeader()->setLabel( tableModule->numCols() - 1, tr( "Dependencias" ) );
    tableModule->setResizePolicy( QTable::AutoOneFit );
    tableModule->setNumRows( 0 );
    tableModule->setNumCols( 3 );
    tableModule->setReadOnly( TRUE );
    tableModule->setSelectionMode( QTable::SingleRow );
    tabLayout_2->addWidget( tableModule );

    btn_descargar = new QPushButton( tab_2, "btn_descargar" );
    tabLayout_2->addWidget( btn_descargar );

    grp_moduleCharger = new QGroupBox( tab_2, "grp_moduleCharger" );
    grp_moduleCharger->setColumnLayout(0, Qt::Vertical );
    grp_moduleCharger->layout()->setSpacing( 6 );
    grp_moduleCharger->layout()->setMargin( 11 );
    grp_moduleChargerLayout = new QVBoxLayout( grp_moduleCharger->layout() );
    grp_moduleChargerLayout->setAlignment( Qt::AlignTop );

    layout1 = new QHBoxLayout( 0, 0, 6, "layout1"); 

    textLabel1 = new QLabel( grp_moduleCharger, "textLabel1" );
    layout1->addWidget( textLabel1 );

    txt_nombremodulo = new QLineEdit( grp_moduleCharger, "txt_nombremodulo" );
    layout1->addWidget( txt_nombremodulo );
    grp_moduleChargerLayout->addLayout( layout1 );

    chk_cargaralinicio = new QCheckBox( grp_moduleCharger, "chk_cargaralinicio" );
    grp_moduleChargerLayout->addWidget( chk_cargaralinicio );

    btn_cargar = new QPushButton( grp_moduleCharger, "btn_cargar" );
    grp_moduleChargerLayout->addWidget( btn_cargar );
    tabLayout_2->addWidget( grp_moduleCharger );
    tabs->insertTab( tab_2, QString::fromLatin1("") );

    TabPage = new QWidget( tabs, "TabPage" );
    TabPageLayout = new QVBoxLayout( TabPage, 11, 6, "TabPageLayout"); 

    grp_dev = new QGroupBox( TabPage, "grp_dev" );
    grp_dev->setColumnLayout(0, Qt::Vertical );
    grp_dev->layout()->setSpacing( 6 );
    grp_dev->layout()->setMargin( 11 );
    grp_devLayout = new QVBoxLayout( grp_dev->layout() );
    grp_devLayout->setAlignment( Qt::AlignTop );

    txtbox_dev = new QTextBrowser( grp_dev, "txtbox_dev" );
    txtbox_dev->setTextFormat( QTextBrowser::AutoText );
    grp_devLayout->addWidget( txtbox_dev );
    TabPageLayout->addWidget( grp_dev );

    btn_exportdevices = new QPushButton( TabPage, "btn_exportdevices" );
    TabPageLayout->addWidget( btn_exportdevices );
    tabs->insertTab( TabPage, QString::fromLatin1("") );

    TabPage_2 = new QWidget( tabs, "TabPage_2" );
    TabPageLayout_2 = new QVBoxLayout( TabPage_2, 11, 6, "TabPageLayout_2"); 

    txt_mem = new QTextBrowser( TabPage_2, "txt_mem" );
    TabPageLayout_2->addWidget( txt_mem );
    tabs->insertTab( TabPage_2, QString::fromLatin1("") );
    frm_mainLayout->addWidget( tabs );

    // actions
    fileNewAction = new QAction( this, "fileNewAction" );
    fileNewAction->setIconSet( QIconSet( image0 ) );
    fileOpenAction = new QAction( this, "fileOpenAction" );
    fileOpenAction->setIconSet( QIconSet( image1 ) );
    fileSaveAction = new QAction( this, "fileSaveAction" );
    fileSaveAction->setIconSet( QIconSet( image2 ) );
    fileSaveAsAction = new QAction( this, "fileSaveAsAction" );
    filePrintAction = new QAction( this, "filePrintAction" );
    filePrintAction->setIconSet( QIconSet( image3 ) );
    fileExitAction = new QAction( this, "fileExitAction" );
    editUndoAction = new QAction( this, "editUndoAction" );
    editUndoAction->setIconSet( QIconSet( image4 ) );
    editRedoAction = new QAction( this, "editRedoAction" );
    editRedoAction->setIconSet( QIconSet( image5 ) );
    editCutAction = new QAction( this, "editCutAction" );
    editCutAction->setIconSet( QIconSet( image6 ) );
    editCopyAction = new QAction( this, "editCopyAction" );
    editCopyAction->setIconSet( QIconSet( image7 ) );
    editPasteAction = new QAction( this, "editPasteAction" );
    editPasteAction->setIconSet( QIconSet( image8 ) );
    editFindAction = new QAction( this, "editFindAction" );
    editFindAction->setIconSet( QIconSet( image9 ) );
    helpContentsAction = new QAction( this, "helpContentsAction" );
    helpIndexAction = new QAction( this, "helpIndexAction" );
    helpAboutAction = new QAction( this, "helpAboutAction" );
    Action = new QAction( this, "Action" );


    // toolbars


    // menubar
    menubar = new QMenuBar( this, "menubar" );


    Archivo = new QPopupMenu( this );
    Archivo->insertSeparator();
    fileExitAction->addTo( Archivo );
    menubar->insertItem( QString(""), Archivo, 1 );

    Ayuda = new QPopupMenu( this );
    helpContentsAction->addTo( Ayuda );
    Ayuda->insertSeparator();
    helpAboutAction->addTo( Ayuda );
    menubar->insertItem( QString(""), Ayuda, 2 );

    languageChange();
    resize( QSize(501, 468).expandedTo(minimumSizeHint()) );
    clearWState( WState_Polished );

    // signals and slots connections
    connect( fileExitAction, SIGNAL( activated() ), this, SLOT( fileExit() ) );
    connect( helpIndexAction, SIGNAL( activated() ), this, SLOT( helpIndex() ) );
    connect( helpContentsAction, SIGNAL( activated() ), this, SLOT( helpContents() ) );
    connect( helpAboutAction, SIGNAL( activated() ), this, SLOT( helpAbout() ) );
    connect( tabs, SIGNAL( currentChanged(QWidget*) ), this, SLOT( changeTab(QWidget*) ) );
    connect( pushButtonKill, SIGNAL( released() ), this, SLOT( kill() ) );
    connect( btn_cargar, SIGNAL( released() ), this, SLOT( cargarModulo() ) );
    connect( btn_descargar, SIGNAL( released() ), this, SLOT( descargarModulo() ) );
    connect( btn_exportdevices, SIGNAL( released() ), this, SLOT( exportdev() ) );
}

/*
 *  Destroys the object and frees any allocated resources
 */
frm_main::~frm_main()
{
    // no need to delete child widgets, Qt does it all for us
}

/*
 *  Sets the strings of the subwidgets using the current
 *  language.
 */
void frm_main::languageChange()
{
    setCaption( tr( "Kernel Info GUI" ) );
    tableProcess->horizontalHeader()->setLabel( 0, tr( "PID" ) );
    tableProcess->horizontalHeader()->setLabel( 1, tr( "Nombre" ) );
    tableProcess->horizontalHeader()->setLabel( 2, tr( "Status" ) );
    tableProcess->horizontalHeader()->setLabel( 3, tr( "Memoria" ) );
    pushButtonKill->setText( tr( "Ma&tar" ) );
    pushButtonKill->setAccel( QKeySequence( tr( "Alt+T" ) ) );
    tabs->changeTab( tab, tr( "&Procesos" ) );
    tableModule->horizontalHeader()->setLabel( 0, tr( "Modulo" ) );
    tableModule->horizontalHeader()->setLabel( 1, tr( "Tam." ) );
    tableModule->horizontalHeader()->setLabel( 2, tr( "Dependencias" ) );
    btn_descargar->setText( tr( "D&escargar modulo seleccionado" ) );
    btn_descargar->setAccel( QKeySequence( tr( "Alt+E" ) ) );
    grp_moduleCharger->setTitle( tr( "Cargar modulo..." ) );
    textLabel1->setText( tr( "Nombre del Modulo" ) );
    txt_nombremodulo->setText( QString::null );
    chk_cargaralinicio->setText( tr( "C&argar cuando se inicie el sistema?" ) );
    chk_cargaralinicio->setAccel( QKeySequence( tr( "Alt+A" ) ) );
    btn_cargar->setText( tr( "&Cargar" ) );
    btn_cargar->setAccel( QKeySequence( tr( "Alt+C" ) ) );
    tabs->changeTab( tab_2, tr( "&Modulos" ) );
    grp_dev->setTitle( tr( "Resumen de dispositivos" ) );
    btn_exportdevices->setText( tr( "E&xportar" ) );
    btn_exportdevices->setAccel( QKeySequence( tr( "Alt+X" ) ) );
    tabs->changeTab( TabPage, tr( "&Dispositivos" ) );
    tabs->changeTab( TabPage_2, tr( "Memori&a" ) );
    fileNewAction->setText( tr( "New" ) );
    fileNewAction->setMenuText( tr( "&New" ) );
    fileNewAction->setAccel( tr( "Ctrl+N" ) );
    fileOpenAction->setText( tr( "Open" ) );
    fileOpenAction->setMenuText( tr( "&Open..." ) );
    fileOpenAction->setAccel( tr( "Ctrl+O" ) );
    fileSaveAction->setText( tr( "Save" ) );
    fileSaveAction->setMenuText( tr( "&Save" ) );
    fileSaveAction->setAccel( tr( "Ctrl+S" ) );
    fileSaveAsAction->setText( tr( "Save As" ) );
    fileSaveAsAction->setMenuText( tr( "Save &As..." ) );
    fileSaveAsAction->setAccel( QString::null );
    filePrintAction->setText( tr( "Print" ) );
    filePrintAction->setMenuText( tr( "&Print..." ) );
    filePrintAction->setAccel( tr( "Ctrl+P" ) );
    fileExitAction->setText( tr( "&Salir" ) );
    fileExitAction->setMenuText( tr( "&Salir" ) );
    fileExitAction->setAccel( QString::null );
    editUndoAction->setText( tr( "Undo" ) );
    editUndoAction->setMenuText( tr( "&Undo" ) );
    editUndoAction->setAccel( tr( "Ctrl+Z" ) );
    editRedoAction->setText( tr( "Redo" ) );
    editRedoAction->setMenuText( tr( "&Redo" ) );
    editRedoAction->setAccel( tr( "Ctrl+Y" ) );
    editCutAction->setText( tr( "Cut" ) );
    editCutAction->setMenuText( tr( "&Cut" ) );
    editCutAction->setAccel( tr( "Ctrl+X" ) );
    editCopyAction->setText( tr( "Copy" ) );
    editCopyAction->setMenuText( tr( "C&opy" ) );
    editCopyAction->setAccel( tr( "Ctrl+C" ) );
    editPasteAction->setText( tr( "Paste" ) );
    editPasteAction->setMenuText( tr( "&Paste" ) );
    editPasteAction->setAccel( tr( "Ctrl+V" ) );
    editFindAction->setText( tr( "Find" ) );
    editFindAction->setMenuText( tr( "&Find..." ) );
    editFindAction->setAccel( tr( "Ctrl+F" ) );
    helpContentsAction->setText( tr( "&Contenido..." ) );
    helpContentsAction->setMenuText( tr( "&Contenido..." ) );
    helpContentsAction->setAccel( QString::null );
    helpIndexAction->setText( tr( "Index" ) );
    helpIndexAction->setMenuText( tr( "&Index..." ) );
    helpIndexAction->setAccel( QString::null );
    helpAboutAction->setText( tr( "&Acerca de..." ) );
    helpAboutAction->setMenuText( tr( "&Acerca de..." ) );
    helpAboutAction->setAccel( QString::null );
    Action->setText( tr( "Unnamed" ) );
    if (menubar->findItem(1))
        menubar->findItem(1)->setText( tr( "Archi&vo" ) );
    if (menubar->findItem(2))
        menubar->findItem(2)->setText( tr( "A&yuda" ) );
}

void frm_main::fileNew()
{
    qWarning( "frm_main::fileNew(): Not implemented yet" );
}

void frm_main::fileOpen()
{
    qWarning( "frm_main::fileOpen(): Not implemented yet" );
}

void frm_main::fileSave()
{
    qWarning( "frm_main::fileSave(): Not implemented yet" );
}

void frm_main::fileSaveAs()
{
    qWarning( "frm_main::fileSaveAs(): Not implemented yet" );
}

void frm_main::filePrint()
{
    qWarning( "frm_main::filePrint(): Not implemented yet" );
}

void frm_main::fileExit()
{
    qWarning( "frm_main::fileExit(): Not implemented yet" );
}

void frm_main::editUndo()
{
    qWarning( "frm_main::editUndo(): Not implemented yet" );
}

void frm_main::editRedo()
{
    qWarning( "frm_main::editRedo(): Not implemented yet" );
}

void frm_main::editCut()
{
    qWarning( "frm_main::editCut(): Not implemented yet" );
}

void frm_main::editCopy()
{
    qWarning( "frm_main::editCopy(): Not implemented yet" );
}

void frm_main::editPaste()
{
    qWarning( "frm_main::editPaste(): Not implemented yet" );
}

void frm_main::editFind()
{
    qWarning( "frm_main::editFind(): Not implemented yet" );
}

void frm_main::helpIndex()
{
    qWarning( "frm_main::helpIndex(): Not implemented yet" );
}

void frm_main::helpContents()
{
    qWarning( "frm_main::helpContents(): Not implemented yet" );
}

void frm_main::helpAbout()
{
    qWarning( "frm_main::helpAbout(): Not implemented yet" );
}

void frm_main::changeTab(QWidget*)
{
    qWarning( "frm_main::changeTab(QWidget*): Not implemented yet" );
}

void frm_main::kill()
{
    qWarning( "frm_main::kill(): Not implemented yet" );
}

void frm_main::cargarModulo()
{
    qWarning( "frm_main::cargarModulo(): Not implemented yet" );
}

void frm_main::descargarModulo()
{
    qWarning( "frm_main::descargarModulo(): Not implemented yet" );
}

void frm_main::exportdev()
{
    qWarning( "frm_main::exportdev(): Not implemented yet" );
}

