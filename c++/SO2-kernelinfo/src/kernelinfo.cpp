...
void kernelinfo::newDoc()
{
    kernelinfo *ed = new kernelinfo;
    ed->setCaption(tr("Qt Example - Application"));
    ed->show();
}

void kernelinfo::choose()
{
...
void kernelinfo::load( const QString &fileName )
{
    QFile f( fileName );
...
void kernelinfo::closeEvent( QCloseEvent* ce )
{
    if ( !e->isModified() ) {
	ce->accept();
	return;
