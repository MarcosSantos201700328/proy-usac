//   Proyecto 1 IPC2,(FIRST)
//   Autor: Erik Vladimir Giron Marquez C#200313492
...
// -[HECHO*] Clasificacion
//  --[FIX] Arreglar orden de muestreo en GenerarReporteClasif
// -** Borrar archivos temporales de calculo despues de usarlos
// -** Cambiar path de entrada y salida a ser elegidos por el usuario
// [BUG] - Arreglar para volver a procesar con opcion 3 en menu
// [BUG] - Bug extrano para salida
// [BUG] - Cuando se intenta procesar datos por segunda vez
// [FIX] - Arreglar salida de datos en pantalla
// [FIX] - Arreglar salida para mes();
// 
...
const char ArchConfed[13] = "confed.bin";
const char ArchIdxConfed[13] = "idx_cnfd.bin";
const char ArchSelec[13] = "selec.bin";
const char ArchIdxSelec[13] = "idx_sel.bin";
const char ArchTorneo[13] = "torneo.bin";
const char ArchJuegos[13] = "juegos.bin";
const char ArchPuntosac[13] = "puntosac.bin";
const char ArchIdxPuntosac[13] = "ipunac.bin";
const char ArchEqYear[13] = "eqyear.bin";
const char ArchDesemp[13] = "desemp.bin";
const char ArchClasif[13] = "clasif.bin";
...
void setInput(char *);
void setOutput(char *);
void msgerr(char*,int);
int ActualizaConfed(confederacion);
int ActualizaTorneo(torneo);
int ActualizaSeleccion(seleccion);
int GeneraReporteClasif(idxclasif *,int);
int GuardarClasif(clasif);
int AcumularPuntosMes(clasif &,idxclasif &);
void InicializarIndice(idxclasif *,int);
int Clasificacion(clasif &c);
int buscaDesemp(desemp&);
int calcularPuntosGanados(desemp &);
int prem_desemp(desemp &d);
int premioME(eq_year &);
int guardarEqA(eq_year &);
int leerPunteoPME(eq_year &);
int eqA(eq_year &);
int ValidarJuego(juego);
int ValidaFecha(intervfecha);
int sizeoffile(FILE *);
char * mes(int);
int printPuntosAc(intervfecha);
int CrearIndicePuntos(void);
int GuardarPuntos(char*, float);
int PuntosAC(intervfecha);
int PClasif(char*,unsigned int,int &);
float FNR(char* idsel);
int CalculaPuntosJuego(juego &);
int CreaJuego(juego &);
int menu(void);
void inicializar(void);
int cmd_creaconfed(confederacion &);
int cmd_creaselec(seleccion &);
int cmd_creatorneo(torneo &);
int cmd_juego(juego &);
int cmd_puntosac(intervfecha &);
int cmd_premio_me(eq_year &);
int cmd_mejor_desemp(desemp &);
int cmd_clasif(clasif &);
int parse(char *);

void QuickSort(idxpuntosac *, int, int);
void QuickSort(int *,int,int);
void QuickSort(idxclasif *, int, int);
char * mes(int);
//***********************setInput():***********************///
...
int parse(char * filename)
{
  ...
  if(!abre_arch(filename)) {
	...
	if(cmd_creaselec(RegSelec) == 0){
	  ...
	  if(ret == 0)
	    sprintf(mensaje,"Linea#%d\nSe Creo la seleccion %s con y se asigno a la %s ",
		    numlinea,RegSelec.nombre,RegSelec.id_conf);
	  else if (ret == 1)
	    sprintf(mensaje,"Linea#%d\nSe Actualizo la seleccion %s con y se asigno a la %s ",
		    numlinea,RegSelec.nombre,RegSelec.id_conf);
	  else
	    sprintf(mensaje,"Linea#%d\n,Imposible crear seleccion %s, no existe federacion %s ",
		    numlinea,RegSelec.nombre,RegSelec.id_conf);
	  msgerr(mensaje,2);
	}
	else
	...
	if(cmd_juego(RegJuego) == 0){
	  ...
	  if(ret == 0){
	    juegoCalculado = true;
	    sprintf(mensaje,"Linea#%d\nTorneo = %s, Juego %d de %s, %d, %s - %d, %s - %d, en %d Minutos",
