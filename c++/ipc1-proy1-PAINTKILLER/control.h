...
Input::Input(int posx, int posy, char *s1) : Windw(posx,posy,(strlen(s1)+11)*5, 12, 0 , 0);
{
	strcpy (label,s1);
//	strcpy (label,s2);	
};
void Input::DrawWindow(void)
{
    ...
    outtextxy(wx,wy,label);
    bar(wx+(strlen(label)*5), wy+2, wx+ww, wy+10);
    MostrarPuntero();
};
...
// quepa dentro del cuadro de entrada.
        setcolor(WHITE);
        if (l < w) outtextxy(wx+(strlen(label)*5),wy+2, input);
        else
        {
                int i = l - w + 1;
                setfillstyle(SOLID_FILL, BLACK);
                bar(wx+(strlen(label)*5), wy+2, wx+ww,  wy+10);
                outtextxy(wx+(strlen(label)*5), wy+2, &input[i]);
        }
    }
...
// algun caracter para borrar.
    ...
    // cabra dentro del cuadro de entrada.
        ...
        bar(wx+(strlen(label)*5), wy+2, wx+ww,  wy+10);
        setcolor(WHITE);
        if (l < w+1) outtextxy(wx+(strlen(label)*5), wy+2, input);
        else
        {
             int i = l - w;
             outtextxy(wx+(strlen(label)*5), wy+2, &input[i]);
